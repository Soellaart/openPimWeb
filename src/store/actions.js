...
const actionList = {
  ...
  executeTableButtonAction: async (itemId, buttonText, where, headers, data) => {
    const query = 'mutation { executeTableButtonAction(' + (itemId ? 'itemId: "' + itemId + '",' : '') + ' buttonText: "' + buttonText + '" where: """' + JSON.stringify(where) + '""" headers: """' + JSON.stringify(headers) + '""" ' + (data ? ',data: """' + data + '"""' : '') + ') { error, compileError, message, data }}'
    const response = await serverFetch(query)
